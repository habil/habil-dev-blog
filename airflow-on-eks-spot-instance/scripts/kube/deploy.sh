...
echo "Airflow Image Repo" $AIRFLOW_REPOSITORY
echo "EFS File System ID" $EFS_FS_ID
echo "EFS Access Point" $EFS_AP
echo "RDS SQL Connection String" $SQL_ALCHEMY_CONN

if [ -z "$AIRFLOW_REPOSITORY" ]; then
  echo "\AIRFLOW_REPOSITORY environement variable is empty."
  exit 1
fi
if [ -z "$EFS_FS_ID" ]; then
  echo "\EFS_FS_ID environement variable is empty."
  exit 1
fi
if [ -z "$EFS_AP" ]; then
  echo "\EFS_AP environement variable is empty."
  exit 1
fi
if [ -z "$SQL_ALCHEMY_CONN" ]; then
  echo "\SQL_ALCHEMY_CONN environement variable is empty."
  exit 1
fi

AIRFLOW_IMAGE=$AIRFLOW_REPOSITORY
AIRFLOW_TAG=latest
...
${SED_COMMAND} "s|{{EFS_FS_ID}}|$EFS_FS_ID|g" \
  ${TEMPLATE_DIRNAME}/volumes.template.yaml > ${DIRNAME}/volumes.yaml
${SED_COMMAND} -i "s|{{EFS_AP}}|$EFS_AP|g" ${DIRNAME}/volumes.yaml
${SED_COMMAND} "s|{{SQL_ALCHEMY_CONN}}|$SQL_ALCHEMY_CONN|g" \
